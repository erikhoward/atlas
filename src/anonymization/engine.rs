...
impl AnonymizationEngine {
    ...
    pub fn new(config: AnonymizationConfig) -> Result<Self> {
        ...
        let detector: Arc<dyn PiiDetector> = if let Some(ref pattern_path) = config.pattern_library {
            let registry = crate::anonymization::detector::patterns::PatternRegistry::from_file(pattern_path)?;
    ...
    pub async fn anonymize_batch(&self, compositions: Vec<Value>) -> Result<Vec<AnonymizedComposition>> {
